import { ServiceRecordBlockChainModel, VehicleInteractionBlockChainModel } from "../models/interaction.model";
...
export const calculateCompositeRating = (vehicle: VehicleBlockChainModel) => {
    ...
    if (vehicle.interaction.length < 5) {
        return 100;
    }

    let compositeRating =   handleCaluclateServiceReocordScore(vehicle) + 
...
export const handleCaluclateAccidentRepairScore = (vehicle: VehicleBlockChainModel) => {
    ...
    const penalty = (PER_ACCIDENT_PENALTY * accidentRepairs.length) + (PER_SEVERITY_PENALTY * severeCount) + (PER_MISSING_DOC_PENALTY * missingDocs);
...
export const handleCalculateTroubleshootRepairScore = (vehicle: VehicleBlockChainModel) => {
    ...
    const penalty = (REPEATED_PANALTY * repeatedIssues) + (UNRESOLVED_PANALTY * unresolvedIssues) + (MISSING_DOC_PENALTY * missingDocs);
